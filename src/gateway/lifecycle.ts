...
export async function startGateway(configPath?: string): Promise<GatewayContext> {
  ...
  // 8. Message router
  const router = new MessageRouter(bridge, sessionMap, securityGate, registry, logger, config.channels, templateEngine, policyEngine, profileEnricher, vaultStore, config.opencode);
