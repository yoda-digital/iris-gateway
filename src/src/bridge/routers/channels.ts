...
export function channelsRouter(deps: ChannelsDeps): Hono {
  const app = new Hono();
  const { registry, logger, pluginTools } = deps;

  app.post("/tool/send-message", async (c) => {
    const parsed = sendMessageSchema.safeParse(await c.req.json());
    if (!parsed.success) {
      return c.json({ error: "Invalid request", details: parsed.error.flatten() }, 400);
    }
    const body = parsed.data;
    const adapter = registry.get(body.channel);
    if (!adapter) return c.json({ error: `Channel not found: ${body.channel}` }, 404);
    try {
      const result = await adapter.sendText({ to: body.to, text: body.text, replyToId: body.replyToId });
      return c.json(result);
    } catch (err) {
      logger.error({ err, channel: body.channel }, "Tool send-message failed");
      return c.json({ error: String(err) }, 500);
    }
  });

  app.post("/tool/send-media", async (c) => {
    ...
    const adapter = registry.get(body.channel);
    if (!adapter) return c.json({ error: `Channel not found: ${body.channel}` }, 404);
    if (!adapter.sendMedia) return c.json({ error: "Channel does not support media" }, 400);
    try {
      const result = await adapter.sendMedia({
        to: body.to, type: body.type, source: body.url,
        mimeType: body.mimeType ?? "application/octet-stream",
        filename: body.filename, caption: body.caption,
      });
      return c.json(result);
    } catch (err) {
      logger.error({ err, channel: body.channel }, "Tool send-media failed");
      return c.json({ error: String(err) }, 500);
    }
  });

  app.post("/tool/channel-action", async (c) => {
    ...
    try {
      ...
      logger.error({ err, channel: body.channel, action: body.action }, "Tool channel-action failed");
      return c.json({ error: String(err) }, 500);
    }
  });

  app.post("/tool/user-info", async (c) => {
    ...
    const adapter = registry.get(body.channel);
    if (!adapter) return c.json({ error: `Channel not found: ${body.channel}` }, 404);
    return c.json({ channel: body.channel, userId: body.userId, capabilities: adapter.capabilities });
  });

  app.get("/tool/list-channels", (c) => {
    const channels = registry.list().map((a) => ({ id: a.id, label: a.label, capabilities: a.capabilities }));
    return c.json({ channels });
  });

  app.post("/tool/plugin/:name", async (c) => {
    const name = c.req.param("name");
    const toolDef = pluginTools?.get(name);
    ...
    try {
      ...
      logger.error({ err, tool: name }, "Plugin tool execution failed");
      return c.json({ error: String(err) }, 500);
    }
  });

  app.get("/tool/plugin-manifest", (c) => {
    if (!pluginTools || pluginTools.size === 0) return c.json({ tools: {} });
    const tools: Record<string, { description: string }> = {};
    for (const [name, def] of pluginTools) tools[name] = { description: def.description };
    return c.json({ tools });
  });
