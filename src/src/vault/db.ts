...
-- Migration: add turn_id/step_index to existing audit_log if not present
CREATE TABLE IF NOT EXISTS _audit_log_migrations (applied TEXT PRIMARY KEY);
...
// Runtime migration for existing databases
export function runAuditLogMigration(db: import("better-sqlite3").Database): void {
  try {
